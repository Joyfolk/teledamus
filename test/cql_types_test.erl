...
encode_test() ->
	...
	?assertEqual(<<6:32, 2:32, 251, 255>>, tdm_cql_types:encode(decimal, #decimal{scale = 2, value = -1025})),
  ...
  ?assertEqual(<<4:32, 127,0,0,1>>, tdm_cql_types:encode(inet, #inet{ip = {127,0,0,1}})), %% , port = 8080
  ?assertEqual(<<16:32, 127:16,0:16,0:16,1:16,1:16,2:16,3:16,4:16>>, tdm_cql_types:encode(inet, #inet{ip = {127,0,0,1,1,2,3,4}})),  % , port = 8080
...
decode_test() ->
  ...
  ?assertEqual({#decimal{scale = 2, value = -1025}, <<>>}, tdm_cql_types:decode(decimal, <<6:32, 2:32, 251, 255>>)),
  ...
  ?assertEqual({#inet{ip = {127,0,0,1}}, <<>>}, tdm_cql_types:decode(inet, <<4:32, 127,0,0,1>>)),
  ?assertEqual({#inet{ip = {127,0,0,1,1,2,3,4}}, <<>>}, tdm_cql_types:decode(inet, <<16:32, 127:16,0:16,0:16,1:16,1:16,2:16,3:16,4:16>>)),
