        ...
        [H | T] ->
            {V, X0} = case H of
                [] -> %% no metadata, use blob
                    tdm_cql1_types:decode(blob, Data);
                {_K, _T, _N, Type} ->
                    tdm_cql1_types:decode(Type, Data);
                U ->
...
check_query_flags(#tdm_query_params{bind_values = Bind, skip_metadata = SkipMetadata, page_size = PageSize, paging_state = PagingState, serial_consistency = SerialConsistency}) ->
    case Bind of
        undefined -> 0;
        [] -> 0;
        _ -> throw({unsupported_protocol_feature, bind_variables, cql1})
    end,
    if SkipMetadata =:= false -> 0; true -> throw({unsupported_protocol_feature, skip_metadata, cql1}) end,
    if PageSize =:= undefined -> 0; true -> throw({unsupported_protocol_feature, paging, cql1}) end,
    if PagingState =:= undefined -> 0; true -> throw({unsupported_protocol_feature, paging, cql1}) end,
    if SerialConsistency =:= undefined -> 0; true -> throw({unsupported_protocol_feature, conditional_update, cql1}) end,
    <<>>.

check_execute_flags(#tdm_query_params{skip_metadata = SkipMetadata, page_size = PageSize, paging_state = PagingState, serial_consistency = SerialConsistency}) ->
    if SkipMetadata =:= false -> 0; true -> throw({unsupported_protocol_feature, skip_metadata, cql1}) end,
    if PageSize =:= undefined -> 0; true -> throw({unsupported_protocol_feature, paging, cql1}) end,
    if PagingState =:= undefined -> 0; true -> throw({unsupported_protocol_feature, paging, cql1}) end,
    if SerialConsistency =:= undefined -> 0; true -> throw({unsupported_protocol_feature, conditional_update, cql1}) end,
...
encode_query_params(Params = #tdm_query_params{consistency_level = Consistency}) ->
    CL = encode_consistency_level(Consistency),
    check_query_flags(Params),
    <<CL/binary>>.


encode_execute_params(Params = #tdm_query_params{consistency_level = Consistency, bind_values = Bind}) ->
    CL = encode_consistency_level(Consistency),
    check_execute_flags(Params),
    Vars = encode_values(Bind),
    <<Vars/binary, CL/binary>>.


encode_batch_query(_Query) ->
...
encode_query(Query, Params) when is_binary(Query) ->
    ...
    P = encode_execute_params(Params),
    <<Q/binary,P/binary>>.