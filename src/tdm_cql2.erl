...
startup_header() ->
    #tdm_header{opcode = ?OPC_STARTUP, type = request, version = 2}.
...
encode_frame_header(#tdm_header{type = Type, version = _Version, flags = Flags, stream = Stream, opcode = OpCode}) ->
    ...
    T = encode_type(Type),
    <<T:1, 2:7/big-unsigned-integer, F:1/binary, Stream:8/big-signed-integer, OpCode:8/big-unsigned-integer>>.
...
encode_frame(Frame, Compression) ->
    ...
    H = encode_frame_header(Header#tdm_header{version = 2}),
        ...
        [H | T] ->
            {V, X0} = case H of
                [] -> %% no metadata, use blob
                    tdm_cql2_types:decode(blob, Data);
                {_K, _T, _N, Type} ->
                    tdm_cql2_types:decode(Type, Data);
                U ->
